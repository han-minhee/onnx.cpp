...
namespace HIP_OP
{
    ...
    __global__ void conv_kernel(
        const T *input_data, const T *weight_data, const T *bias_data, T *output_data,
        ...
        for (size_t m = 0; m < M / group; ++m)
        {
            ...
            for (size_t c = 0; c < C / group; ++c)
            {
                ...
                for (size_t i = 0; i < 4; ++i)
                {
                    ...
                    {
                        transformed_output[i][j] = transformed_input[i][j] * transformed_kernel_data[kernel_idx + i * 4 + j];
                    }
                }
                                                    ...
                                                    const std::unordered_map<std::string, Node::AttributeValue> &attributes, Device *device)
    {
        ...
        case TensorDataType::FLOAT64:
            if (use_winograd)
                return executeWinogradConv<double>(X, W, B, Y, pads, strides, dilations, group, N, C, H, W_in, M, kH, kW, H_out, W_out);
            else
